...
var CascadeButtonManager = function () {
    //creates buttons with unique id
    
    this.amount = 0;
    
    this.createButton = function ( tooltip, symbol ) {
        var cascadebutton = new CascadeButton ( this.amount, tooltip, symbol );
        this.amount ++;
...
var CascadeButton = function ( index, tooltip, symbol ) {
    
    this.index = index;
    this.tooltip = tooltip;
    
    this.symbol = ( symbol !== undefined ) ? symbol : null;
    ...
    this.SIZE = ( symbol !== undefined )? 17 : 12;
    this.EDGEROOM = 2;
    this.LINEWIDTH = 0;//1.5;
    ...
    this.canvas.style.top = ( symbol !== undefined )? "4px" : "2px";
    ...
    this._redraw = function () {
        ...
        this.ctx.fillStyle = ( this.symbol !== null )? GLOBALDATA.CLR_DARKRED : this.BACKCLR;
        this.ctx.fillRect ( 0, 0, this.SIZE, this.SIZE );

        
        if ( this.symbol !== null ) {
            this.ctx.font = MAIN._styledata.getCanvasFontStyle ( "cascade_symbol" );
            this.ctx.textAlign = "center";
            var tclr = "#ffffff";
            if ( this.state === this.STATE_OPEN ) tclr = GLOBALDATA.CLR_GOLD;
            if ( this.over ) tclr = "#ff0000";
            this.ctx.fillStyle = tclr;
            //this.ctx.fillStyle = ( this.state === this.STATE_OVER )? "#ffffff" : "#FFCD00";
            this.ctx.fillText ( this.symbol, this.SIZE / 2, this.SIZE - 3 );
        }
        else {
