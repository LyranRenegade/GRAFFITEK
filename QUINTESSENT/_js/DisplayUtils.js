...
function averageColor ( cols ) {
    ...
    var rs, gs, bs;
    for ( i = 0; i < cl; i ++ ) {
        rs = cols [ i ].substring ( 1, 3 );
        gs = cols [ i ].substring ( 3, 5 );
        bs = cols [ i ].substring ( 5 );
        rv += parseInt ( rs, 16 );
        gv += parseInt ( gs, 16 );
        bv += parseInt ( bs, 16 );
    }
    
    rs = ( Math.round ( rv / cl ) ).toString ( 16 );
    gs = ( Math.round ( gv / cl ) ).toString ( 16 );
    bs = ( Math.round ( bv / cl ) ).toString ( 16 );
    
    return "#" + rs + gs + bs;
}


function _drawEllipse ( ctxid, centerX, centerY, width, height, fillcolor ) {
...
function drawQuickCirle ( x, y, radius, ctxid ) {
    var ctx = getContext ( ctxid );
...
function drawCirle ( x, y, radius, ctxid, clear, fillcolor, strokecolor ) {
    var ctx = getContext ( ctxid );
    if ( clear ) ctx.clearRect ( 0, 0, CLEARSIZE, CLEARSIZE );
...
function getRGBA ( clrstring, alpha ) {
    //a between 0 and 1
    var rs = clrstring.substring ( 1, 3 );
    var gs = clrstring.substring ( 3, 5 );
    var bs = clrstring.substring ( 5 );
    var rv = parseInt ( rs, 16 );
    var gv = parseInt ( gs, 16 );
    var bv = parseInt ( bs, 16 );
     
    return "rgba(" + String ( rv ) + "," + String ( gv ) + "," + String ( bv ) + "," + String ( alpha ) + ")";
